...
a tail pointing back at it. Some panels below show the widget library's sample
data rather than a live reading, and say so.
...
<table>
  ...
  <tr>
    <td valign="top">
      ...
      <sub><b>Now Playing</b> - the empty state, which is what you get when no player can be read.</sub>
    </td>
...
unsigned caller. So Docket asks the players directly over Apple Events: Spotify
...
[Releases page](https://github.com/nparashar150/docket/releases), unzip it
and move `Docket.app` to `/Applications`. If there is no release there yet,
...
Docket is an `LSUIElement` accessory app: **no Dock tile and no app window of its own**. Its
...
- **Apple Events need consent.** Reading Spotify, Music or a browser tab
  requires approving Docket under Privacy & Security ▸ Automation, and the
